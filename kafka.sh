...
KAFKA_DIR="/opt/kafka/kafka_2.13-3.8.0"
...
kafkaStart() {
    echo "Starting kafka..."
    rm "${KAFKA_LOG}"
    nohup bash "$KAFKA_DIR/bin/kafka-server-start.sh" "$KAFKA_DIR/config/kraft/server.properties" >"${KAFKA_LOG}" 2>&1 &
    sleep 2
...
kafkaStop() {
        echo "Stopping kafka..."
        PIDS=$(ps -ef | grep "$KAFKA_DIR/config/kraft/server.properties" | grep kafka | grep java | grep -v grep | awk {'print $2'})
